...
j=esurn1(esurn2(ni)+k);
    ...
    for i=1:2
        if (size(T,1)==size(O,1))&&(k==nec)&&(i==2)
            K(1,1)=mobility(V(i,k,ni))*kmap(elem(j,5),2);
            K(1,2)=mobility(V(i,k,ni))*kmap(elem(j,5),3);
            K(2,1)=mobility(V(i,k,ni))*kmap(elem(j,5),4);
            K(2,2)=mobility(V(i,k,ni))*kmap(elem(j,5),5);
