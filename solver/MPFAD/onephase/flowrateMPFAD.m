...
function [flowrate, flowresult,velocity]=flowrateMPFAD(p,w,s,Kde,Ded,Kn,Kt,Hesq,nflag,auxflag,mobility)

global coord esurn1 esurn2 bedge inedge centelem bcflag


%invertendo a ordem de mobilyt para mobily de inedge + bedge (COISA DO
%FERNANDO)
...
for iface=1:size(inedge,1)
    ...
    velocity(iface+size(bedge,1)) = mobility(iface+size(bedge,1))*flowrate(iface+size(bedge,1))/ nor;
    %Attribute the flow rate to "flowresult"
    %On the left:
