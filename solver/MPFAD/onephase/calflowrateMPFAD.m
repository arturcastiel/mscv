...
function [flowrate, flowresult,velo]=calflowrateMPFAD(p,w,s,Kde,Ded,Kn,Kt,Hesq,nflag,auxflag,mobility)
...
for ifacont=1:size(bedge,1);
    ...
    if bedge(ifacont,5)<200 % se os nós esteverem na fronteira de DIRICHLET
        ...
        velo(ifacont) = flowrate(ifacont)/nor;
    else
...
for iface=1:size(inedge,1)
    ...
    velo(iface+size(bedge,1)) = flowrate(iface+size(bedge,1))/nor;
    %Attribute the flow rate to "flowresult"
    %On the left:
